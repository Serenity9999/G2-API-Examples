  ...
  <div class="wrapper">
    ...
    <ul class="tab-bar">
      <li class="set-wrapper" @click="drawer = true">
        <a href="javascript:;" class="set">设置音效</a>
      </li>
      <li class="over" @click="handleOver"></li>
    </ul>
    <el-drawer title="设置音效" :visible.sync="drawer" direction="ltr">
      <div class="pl20 pr20 mb20 flex jcb">
        <span class="mr10">音效</span>
        <el-select value-key="soundId" v-model="soundEffectSelected" placeholder="请选择音效">
          <el-option
            :disabled="!preloadFlag"
            v-for="item in soundEffectOptions"
            :key="item.soundId"
            :value="item"
            :label="item.label"
          />
      ...
      <div class="pl20 pr20 mb20 flex jcb">
        ...
        <div class="f1">
          <el-slider
            :disabled="!soundEffectSelected"
            @input="changeVolume(soundEffectSelected, primaryVolume)"
            :max="100"
      ...
      <div class="t-center">
        <el-button
          ...
          :disabled="!soundEffectSelected"
          round
          @click="playEffect(soundEffectSelected)"
          >播放</el-button
        >
        <el-button
          type="warning"
          :disabled="!soundEffectSelected"
          round
          @click="pauseEffect(soundEffectSelected)"
          >暂停</el-button
        >
        <el-button
          type="success"
          :disabled="!soundEffectSelected"
          round
          @click="resumeEffect(soundEffectSelected)"
          >恢复</el-button
        >
        <el-button
          type="danger"
          :disabled="!soundEffectSelected"
          round
          @click="stopEffect(soundEffectSelected)"
          >停止</el-button
        >
      </div>
    <br/>
      <div class="pl20 pr20 mb20 flex jcb">
        <span class="mr10">音效</span>
        <el-select value-key="soundId" v-model="extraSoundEffectSelected" placeholder="请选择音效">
          <el-option
            :disabled="!preloadFlag"
            v-for="item in soundEffectOptions"
            :key="item.soundId"
      ...
      <div class="pl20 pr20 mb20 flex jcb">
        ...
        <div class="f1">
          <el-slider
            :disabled="!extraSoundEffectSelected"
            @input="changeVolume(extraSoundEffectSelected, extraPrimaryVolume)"
            :max="100"
            v-model="extraPrimaryVolume"
          ></el-slider>
        </div>
      </div>
      <div class="t-center">
        <el-button
          ...
          :disabled="!extraSoundEffectSelected"
          round
          @click="playEffect(extraSoundEffectSelected)"
          >播放</el-button
        >
        <el-button
          type="warning"
          :disabled="!extraSoundEffectSelected"
          round
          @click="pauseEffect(extraSoundEffectSelected)"
          >暂停</el-button
        >
        <el-button
          type="success"
          :disabled="!extraSoundEffectSelected"
          round
          @click="resumeEffect(extraSoundEffectSelected)"
          >恢复</el-button
        >
        <el-button
          type="danger"
          :disabled="!extraSoundEffectSelected"
          round
          @click="stopEffect(extraSoundEffectSelected)"
          >停止</el-button
        >
      </div>
    </el-drawer>
    ...
    export default {
        ...
        data() {
            return {
                ...
                max: 4,
                preloadFlag: false,
                soundEffectSelected: null,
                extraSoundEffectSelected: null,
                soundEffectOptions: [
                    {
                        soundId: 1,
                        filePath:
                            'https://yunxin-g2-assets.oss-cn-hangzhou.aliyuncs.com/web/auido/yrybz.aac',
                        label: 'yrybz.aac',
                    },
                    {
                        soundId: 2,
                        filePath:
                            'https://yunxin-g2-assets.oss-cn-hangzhou.aliyuncs.com/web/auido/蓝天白云.mp3',
                        label: '蓝天白云.mp3',
                    },
                    {
                        soundId: 3,
                        filePath:
                            'https://yunxin-g2-assets.oss-cn-hangzhou.aliyuncs.com/web/auido/biu.mp3',
                        label: 'biu.mp3',
                    },
                    {
                        soundId: 4,
                        filePath:
                            'https://yunxin-g2-assets.oss-cn-hangzhou.aliyuncs.com/web/auido/smlt.flac',
                        label: 'smlt.flac',
                    },
                ],
            };
        ...
        methods: {
            playEffect(soundEffect) {
                this.localStream
                    .playEffect({
                        filePath: soundEffect.filePath,
                        soundId: soundEffect.soundId,
                        cycle: 1,
                    })
                    .then((res) => {
                        console.warn('播放音效文件成功');
                    })
                    .catch((err) => {
                        console.error('播放音效文件失败: ', err);
                    });
            },
            pauseEffect(soundEffect) {
                this.localStream
                    .pauseEffect(soundEffect.soundId)
                    .then(() => {
                        console.warn('暂停音效文件成功');
                    })
                    .catch((err) => {
                        console.error('暂停音效文件失败: ', err);
                    });
            },
            resumeEffect(soundEffect) {
                this.localStream
                    .resumeEffect(soundEffect.soundId)
                    .then(() => {
                        console.warn('恢复音效文件成功');
                    })
                    .catch((err) => {
                        console.error('恢复音效文件失败: ', err);
                    });
            },
            stopEffect(soundEffect) {
                this.localStream
                    .stopEffect(soundEffect.soundId)
                    .then(() => {
                        console.warn('停止音效文件成功');
                    })
                    .catch((err) => {
                        console.error('停止音效文件失败: ', err);
                    });
            },
            changeVolume(soundEffect, volume) {
                if (!this.localStream) {
                ...
                this.localStream
                    .setVolumeOfEffect(soundEffect.soundId, volume)
                    .then(() => {
                        console.warn('设置音量成功');
                    })
                    .catch((err) => {
            ...
            joinChannel(token) {
                ...
                this.client
                    ...
                    .then((data) => {
                        ...
                        // 预加载音效
                        Promise.all(
                            this.soundEffectOptions.map((item) => {
                                return this.localStream.preloadEffect(
                                    item.soundId,
                                    item.filePath
                                );
                            })
