<template>
  <div class="wrapper">
    ...
    <!--底层栏-->
    <ul class="tab-bar">
      <li class="set-wrapper" @click="changeRole">
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.client.on('client-role-changed', (evt) => {
                console.warn(`当前角色为：${evt.role}`);
            })
        ...
        methods: {
            changeRole() {
                this.client.setClientRole(this.role)
                    .then(() => {
                        // console.warn('当前角色为：', this.role);
                        if (this.role === 'host') {
                            if (!this.repeatPublish){
                                this.publish()
                            }
                            this.btnContent = '切换为观众';
                            this.role = 'audience'
                        } else {
                            this.btnContent = '切换为主播';
                            this.role = 'host'
                        }
                        // 避免首次加入页面时publish两次
                        if (this.repeatPublish){
                            this.repeatPublish = false;
                        }
                    })
                    .catch((err) => {
