    ...
    <div class="content">
      ...
      <el-card class="control-container" body-style="backgroundColor: initial">
        <el-row>
          ...
          <el-col :span="8">
            <el-button type="primary" @click="registerAIDenoise"
              >注册</el-button
            >
          </el-col>
          <el-col :span="8">
    ...
    export default {
        ...
        data() {
            return {
                ...
                color: '#e7ad3c',
                level: 1,
                AIDenoisePluginConfig: {
                    simd: {
                        key: 'AIDenoise',
                        pluginObj: AIDenoise,
                        wasmUrl:
                            'https://yx-web-nosdn.netease.im/sdk-release/NIM_Web_AIDenoise_simd_v4.6.25.wasm' +
                    ...
                    nosimd: {
                        ...
                        pluginObj: AIDenoise,
                        wasmUrl:
                            'https://yx-web-nosdn.netease.im/sdk-release/NIM_Web_AIDenoise_nosimd_v4.6.25.wasm' +
                            `?download=${new Date().valueOf()}`,
                    },
                },
                registerDenoise: false,
                openDenoise: false,
            };
        },
        mounted() {
        ...
        methods: {
            async registerAIDenoise() {
                if (!this.localStream) {
                    return;
                }
                const type = (await wasmFeatureDetect.simd()) ? 'simd' : 'nosimd';
                const denoiseConfig = this.AIDenoisePluginConfig[type];
                this.localStream.registerPlugin(denoiseConfig);
            },
            openAIDenoise() {
            ...
            initLocalStream() {
                ...
                this.localStream.on('plugin-load', (name) => {
                    if (name === 'AIDenoise') {
                        console.log('register AIdenoise success');
                        // AI降噪注册成功
                        this.registerDenoise = true;
                    }
                });
