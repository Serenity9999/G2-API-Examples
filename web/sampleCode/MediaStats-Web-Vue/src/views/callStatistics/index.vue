  ...
  <div class="wrapper">
    <div class="content">
      ...
      <div class="main-window" ref="large">
          <div class="statistics-box" v-if="showStatistics">
              <div>频道中的用户数：{{UserCount}}</div>
              <div>频道时长：{{Duration}}</div>
              <div>接收码率：{{RecvBitrate}}</div>
              <div>发送码率：{{SendBitrate}}</div>
              <div>接收总字节数：{{RecvBytes}}</div>
              <div>发送总字节数：{{SendBytes}}</div>
              <div>上行可用带宽：{{OutgoingAvailableBandwidth}}</div>
    ...
    <ul class="tab-bar">
      ...
      <li class="set-wrapper" @click="toggleShowStatistics">
        <a href="javascript:;" class="set">{{showStatistics ? '关闭统计' : '显示统计'}}</a>
      </li>
    ...
    export default {
        ...
        data() {
            return {
                ...
                OutgoingAvailableBandwidth: null,
                txRtt: null,
                RecvBitrate: null,
                SendBitrate: null,
                RecvBytes: null,
            ...
            getCallStatistics() {
                ...
                this.timer = setInterval(() => {
                    Promise.all([
                        ...
                        this.OutgoingAvailableBandwidth = stats1.OutgoingAvailableBandwidth;
                        this.txRtt = stats1.txRtt;
                        this.Duration = stats2.Duration;
                        this.RecvBitrate = stats2.RecvBitrate;
                        this.RecvBytes = stats2.RecvBytes;
                        this.SendBitrate = stats2.SendBitrate;
                        this.SendBytes = stats2.SendBytes;
                        this.UserCount = stats2.UserCount;
                    }).catch(err => {
                        console.warn('getCallStatistics Error: ' + err);
                    })
                }, 1000);
            },
