<template>
  <div class="wrapper">
    <div class="content">
      ...
      <div class="sub-window-wrapper">
        ...
        <template v-if="remoteStreams.length">
          <div v-for="item in remoteStreams" :key="item.stream.getId()">
            <div class="sub-window" :data-uid="item.stream.getId()" ref="small"></div>
            <!-- 小窗口底层栏 -->
            <ul class="tab-bar">
              <li
                :class="{
                ...
                @click="muteOrUnmuteAudio(item.stream.getId())"
              ></li>
              <li
                :class="{
                  video: true,
                ...
                @click="muteOrUnmuteVideo(item.stream.getId())"
              ></li>
            </ul>
          </div>
        </template>
    ...
    <!--底层栏-->
    <ul class="tab-bar">
      <li :class="{ audio: true, muteAudio }" @click="muteOrUnmuteAudio()"></li>
      <li class="over" @click="handleOver"></li>
      <li :class="{ video: true, muteVideo }" @click="muteOrUnmuteVideo()"></li>
    </ul>
  </div>
</template>
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.client.on('peer-leave', (evt) => {
                ...
                this.remoteStreams = this.remoteStreams.filter(
                    (item) => !!item.stream.getId() && item.stream.getId() !== evt.uid
                );
            });
        ...
        methods: {
            remoteMuteMedia(id, mediaType) {
                console.log(`id:${id},mediaType:${mediaType}`, this.remoteMute);
                if (this.remoteMute.length > 0) {
                    switch (mediaType) {
                    case 'audio':
                        return this.remoteMute.find((item) => item.id === id).muteAudio;
                    case 'video':
                        return this.remoteMute.find((item) => item.id === id).muteVideo;
                    }
                }
            },
            getToken() {
            ...
            muteOrUnmuteAudio(id) {
                ...
                if (!id) {
                    ...
                    muteAudio = remoteMuteItem.muteAudio;
                }

                if (muteAudio) {
                    console.warn('muteAudio');
                    stream
                        .muteAudio()
                        .then(() => {
                            console.warn('muteAudio成功');
                        })
                        .catch((err) => {
                            console.warn('muteAudio失败:', err);
                            message('muteAudio失败');
                        });
                } else {
                    console.warn('unmuteAudio');
                    if (!stream) {
                        message('当前不能unmuteAudio');
                        return;
                    }
                    stream
                        .unmuteAudio()
                        .then(() => {
                            console.warn('unmuteAudio成功');
                        })
                        .catch((err) => {
                            console.warn('unmuteAudio失败:', err);
                            message('unmuteAudio失败');
                        });
                }
            },
            muteOrUnmuteVideo(id) {
                let stream = '';
                let muteVideo = '';
                if (!id) {
                    stream = this.localStream;
                    ...
                    muteVideo = remoteMuteItem.muteVideo;
                }
                if (muteVideo) {
                    console.warn('muteVideo');
                    stream
                        .muteVideo()
                        .then(() => {
                            console.warn('muteVideo成功');
                        })
                        .catch((err) => {
                            console.warn('muteVideo失败:', err);
                            message('muteVideo失败');
                        });
                } else {
                    console.warn('unmuteVideo');
                    if (!this.localStream) {
                        message('当前不能unmuteVideo');
                        return;
                    }
                    stream
                        .unmuteVideo()
                        .then(() => {
                            console.warn('unmuteVideo成功');
                        })
                        .catch((err) => {
                            console.warn('unmuteVideo失败:', err);
                            message('unmuteVideo失败');
                        });
                }
            },
            handleOver() {
