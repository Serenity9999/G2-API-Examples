<template>
  <div class="wrapper">
    ...
    <!--底层栏-->
    <ul class="tab-bar">
      <li class="set-wrapper" @click="switchStreamType">
        <a href="javascript:;" class="set">{{btnContent}}</a>
      </li>
    ...
    export default {
        ...
        data() {
            return {
                ...
                remoteStreams: [],
                max: 4,
                btnContent: '订阅小流', // 订阅默认接收大流
                currentStreamType: false, // false是大流 true是小流
                isDualStream: false, // 是否开启双流
            };
        },
        mounted() {
            // 初始化音视频实例
            console.warn('初始化音视频sdk');
            this.isDualStream = JSON.parse(this.$route.query.dualStream)
            window.self = this;
            ...
            this.client.on('track-low-init-fail', (evt) => {
                console.warn(`${evt.mediaType} 小流创建失败`);
            })
            this.client.on('track-low-init-success', (evt) => {
                console.warn(`${evt.mediaType} 小流创建成功`);
            })
        ...
        methods: {
            switchStreamType(){
                this.currentStreamType = !this.currentStreamType;
                if (this.currentStreamType){ // 设置为小流
                    this.setRemoteVideoStreamType(NERTC.STREAM_TYPE.LOW);
                    this.btnContent = '订阅大流';
                } else { // 设置为大流
                    this.setRemoteVideoStreamType(NERTC.STREAM_TYPE.HIGH);
                    this.btnContent = '订阅小流';
                }
            },
            setRemoteVideoStreamType(type){
                this.remoteStreams.forEach((item) => {
                    this.client.setRemoteVideoStreamType(item, type)
                })
            },
            getToken() {
                return getToken({
                ...
                this.client
                    ...
                    .then((data) => {
                        ...
                        if (this.isDualStream){
                            ...
                            this.client.enableDualStream({ screen: false, video: false })
                        }
