...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            this.extraClient = NERTC.createClient({
                appkey: config.appkey,
                debug: true,
            });
            //监听事件
            ...
            this.getToken('client')
                .then((token) => {
                    this.joinChannel(token, 'client');
                })
                .catch((e) => {
                    message(e);
                    console.error(e);
                });
            this.getToken('extraClient')
                .then((token) => {
                    this.joinChannel(token, 'extraClient');
                })
                .catch((e) => {
                    message(e);
                    console.error(e);
        ...
        methods: {
            getToken(client) {
                return getToken({
                    uid: client === 'client' ? this.localUid : this.extraUid,
                    appkey: config.appkey,
                    appSecret: config.appSecret,
                    channelName: client === 'client' ? this.$route.query.channelName : this.$route.query.extraChannelName,
                }).then(
                    (token) => {
                        return token;
                    },
            ...
            joinChannel(token, client) {
                if (!this[client]) {
                    message('内部错误，请重新加入房间');
                    return;
                }
                console.info('开始加入房间: ', client === 'client' ? this.$route.query.channelName : this.$route.query.extraChannelName);
                this[client]
                    .join({
                        channelName: client === 'client' ? this.$route.query.channelName : this.$route.query.extraChannelName,
                        uid: client === 'client' ? this.localUid : this.extraUid,
                    ...
                    .then((data) => {
                        ...
                        if (client === 'client') {this.initStream('localStream');} else {this.initStream('extraStream');}
                    })
                    .catch((error) => {
            ...
            initStream(stream) {
                //初始化本地的或额外Stream实例，用于管理本端的或额外音视频流
                this[stream] = NERTC.createStream({
                    uid: stream === 'localStream' ? this.localUid : this.extraUid,
                    audio: true, //是否启动mic
                ...
                //设置本地或额外音频质量
                this[stream].setAudioProfile('speech_low_quality');
                //启动媒体，打开实例对象中设置的媒体设备
                this[stream]
                    .init()
                    .then(() => {
            ...
            publish(stream) {
                ...
                const client = stream === 'localStream' ? 'client' : 'extraClient';
                //发布本地媒体给房间对端
                this[client]
                    .publish(this[stream])
                    .then(() => {
                    ...
                    .catch((err) => {
                        if (stream === 'localStream') {
                            ...
                            console.error('额外 publish 失败: ', err);
                            message('额外 publish 失败');
                        }
                    });
            },
